...
export default class PciProjectNewPaymentRegisterCtrl {
  ...
  $onInit() {
    ...
    this.authorizedPaymentMethods.list = registerablePaymentMethods.sort((methodA, methodB) => {
      const methodAIndex = mappedPreferredMethodOrder.indexOf(methodA.paymentType);
      const methodBIndex = mappedPreferredMethodOrder.indexOf(methodB.paymentType);
      return methodAIndex - methodBIndex;
    });

    this.authorizedPaymentMethods.chunks = chunk(this.authorizedPaymentMethods.list, 2);
